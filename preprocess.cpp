...
/*
inline int _num_threads(int nthreads, int max_possible){

//TODO: you don't need to handle max possible here. You can check the index within the thread
#ifdef _OPENMP
    ...
    if (nthreads==-1)
        ...
        throw std::invalid_argument("ERROR: Invalid nthreads!");

#endif

    nthreads = std::min(nthreads, static_cast<int>(max_possible));
...
template <class T>
inline const bool _approx_equal (T val1, T val2){
    return std::abs(val1 - val2) < EPSILON;
}

template <class T>
inline const size_t _tquant_distance(const std::vector<T>& tquant, int tquant_i, int tquant_j, T t_min,  T t_max){
...
class preprocessor{
    ...
    public :
        ...
        preprocessor(const T* data_, const size_t nrows_, const size_t ncols_, const boundary_info* bndry_info_, T* out_data_, const T* quant_arr, const size_t quant_per_column_, const size_t t_start_idx_, const size_t t_end_idx_, const size_t delta_idx_, const size_t pat_col_idx_):
            data(data_),
            nrows(nrows_),
            ncols(ncols_),
            ...
            tquant(std::vector<T>(
                        ...
                        quant_arr+(t_start_idx_+1)*quant_per_column_)),
            quant(quant_arr),
        ...
        inline void quantize_column(size_t col_idx){
            auto column_quant = std::vector<T>(
                        ...
                        quant + (col_idx+1)* quant_per_column);

            //std::vector<T> column_quant{ 10, 20, 30 };
            ...
            for (size_t row_idx = 0; row_idx < nrows; ++row_idx){
                ...
                if (col_idx == t_start_idx  ||
                    ...
                    /*
                    for (auto i:column_quant){
                        std::cout<<i<<", ";
                    } std::cout<<std::endl;

                    std::vector<T> __vals {5, 10, 15, 20, 25, 30, 35};
                    
                     for (auto _val:__vals){
                        auto val_iter = std::lower_bound (
                                column_quant.begin(), 
                                column_quant.end(), 
                                _val);

                        val_iter = max(--val_iter, column_quant.begin());
                        std::cout<<"val:"<<_val<<" "<<"quant:"<<*val_iter<<std::endl;
                    }
                    return;
                    */
        ...
        void quantize_non_time_columns(int nthreads = -1){

            temp_quantized_data  = new T [nrows*ncols]; 
            //int nthreads_ = _num_threads(nthreads, ncols);
            if (nthreads > 0){
        ...
        void preprocess(int nthreads=-1){

            quantize_non_time_columns(nthreads);

            //int nthreads_ = _num_threads(nthreads, npatients);
            if (nthreads>0){
                omp_set_num_threads(nthreads);
            }
...
extern "C" void free_boundary_info(boundary_info* bndry_info){
    delete bndry_info; 
}
                    ...
                    const size_t nthreads)
{
    ...
    preprocessor<double> preprocessor_ (data, nrows, ncols, bndry_info, out_data, quant_arr, quant_per_column, t_start_idx, t_end_idx, delta_idx, pat_col_idx);
    
    try {
        preprocessor_.preprocess(nthreads);
    } catch (std::invalid_argument& e){
        std::cout<<err.str();
        //throw;
    }
...
class pat_lb_ub_calculator{
    ...
    public:
        ...
        pat_lb_ub_calculator(const T* data_, const size_t nrows_, const size_t ncols_, const size_t npatients_, const T* quant_arr, const size_t quant_per_column, const size_t t_start_idx_, const size_t pat_col_idx_, size_t t_end_idx_):
            data(data_),
            nrows(nrows_),
            ncols(ncols_),
            ...
            tquant(std::vector<T>(
                        ...
                        quant_arr+(t_start_idx_+1)*quant_per_column)),
            t_start_idx(t_start_idx_),
            pat_col_idx(pat_col_idx_),
            t_end_idx(t_end_idx_),
    ...
    private:
         ...
         /*
        inline std::tuple<int, int, int> tpart_i_j_out_len (int lb, int ub){
            std::pair<int,int> tpart_i_j = _get_range_sorted<T>(tpart, data[lb*ncols+t_start_idx], data[ub*ncols+t_start_idx]);
            int tpart_i = tpart_i_j.first;
            int tpart_j = tpart_i_j.second;
            int out_len = _out_len(lb, ub, tpart_i, tpart_j);
            return std::make_tuple(tpart_i, tpart_j, out_len);
...
extern "C" boundary_info* get_boundaries(const void* data_v, size_t nrows, size_t ncols, size_t npatients, size_t pat_col_idx, size_t t_start_idx, size_t t_end_idx, const void* quant_v, size_t quant_per_column){
    const double* data        = (double *) data_v;
    const double* quant_arr   = (double *) quant_v;
       
    pat_lb_ub_calculator<double> pat_lb_ub_calculator_ = pat_lb_ub_calculator<double>(data, nrows, ncols, npatients, quant_arr, quant_per_column, t_start_idx, pat_col_idx, t_end_idx);
...
template <class T>
inline void _compute_quant(const T* data, size_t nrows, size_t ncols, size_t t_start_idx, size_t t_end_idx, size_t pat_idx, size_t delta_idx, T* quant, size_t quant_per_column, int nthreads){
    ...
    for (size_t col_idx = 0; col_idx<ncols; ++col_idx){
        if (col_idx == t_end_idx || col_idx == pat_idx || col_idx == delta_idx){
            continue;
        }
        ...
        for (size_t i=0; i<quant_per_column; ++i){
            quant[col_idx*quant_per_column+i] = vals[static_cast<int>(nom_unique*i/quant_per_column)];
        }
...
template <class T>
inline void _fill_quants (T* quant, size_t quant_per_column, size_t col_idx,
    ...
    for (size_t i = 0; i<quant_per_column; ++i){
        const T quant_to_select = static_cast<T>(i)/quant_per_column;
        ...
        //std::cout << idx << "   " <<unique[idx]<<std::endl;
    }
}
...
template <class T>
inline void _compute_quant_weighted(const T* data, size_t nrows, size_t ncols, size_t t_start_idx, size_t t_end_idx, size_t pat_idx, size_t delta_idx, T* quant, size_t quant_per_column, int nthreads){
    ...
    for (size_t col_idx = 0; col_idx<ncols; ++col_idx){
        ...
        _fill_quants (quant, quant_per_column, col_idx,
...
extern "C" void compute_quant(const void* data_v, size_t nrows, size_t ncols, size_t t_start_idx, size_t t_end_idx, size_t pat_idx, size_t delta_idx, void* quant_v, size_t quant_per_column, bool weighted, int nthreads){

    const double* data = (double *) data_v;
    double* quant      = (double *) quant_v;

    if (weighted){
        _compute_quant_weighted<double> (data, nrows, ncols, t_start_idx, t_end_idx, pat_idx, delta_idx, quant, quant_per_column, nthreads);
    }
    else{
        _compute_quant<double> (data, nrows, ncols, t_start_idx, t_end_idx, pat_idx, delta_idx, quant, quant_per_column, nthreads);
    }
}

extern "C" void fix_data_on_boundaries(void* data_v, size_t nrows, size_t ncols, const void* quant_idx_v, const void* quant_v, size_t quant_per_column, int nthreads){
    ...
    for (size_t col_idx = 0; col_idx < ncols; ++col_idx){
        ...
        /*for (int i = 0; i < column_quant.size(); ++i){

                std::cout<< column_quant[i] << ", ";
